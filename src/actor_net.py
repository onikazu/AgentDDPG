...
class ActorNet:
    ...
    def __create_graph(self):
        # 状態空間の次元数のインプットのための場所用意
        state_input = tf.placeholder(tf.float32, [None, self.num_states])

        # tf.varは変数　
        # tf.truncated_normalは第一引数に生成するtensorの形. 正規分布からランダムに数を入れる
        # tf.constantは定数のテンソルを示す
        # 各層を下の部分は示しているがまだ連結されてはいない
        ...
        # tf.matmulはテンソルの掛け算を行っている部分
        # tf.nn.reluの第一引数はフィルタやバイアスを足しこんだ結果(Wx + b)のテンソルを利用する
    ...
    # state_batchにはstateを1*num_statesの２次元なむパイ配列に変換したものが入る
    def forward_learned_net(self, state_batch):
        # sess.runはモデルの実行する部分（独立で）
